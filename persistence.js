...
(function () {
  ...
  // Stable task keys keep completion records attached when Mom edits or reorders tasks.
  ...
  function adminTarget() {
    if (!PLAYER_NAMES.includes(state.adminTarget)) state.adminTarget = 'Alex';
    return state.adminTarget;
  }
  ...
  window.selectAdminTarget = function selectAdminTarget(name) {
  ...
  window.addManagedTask = function addManagedTask() {
    ...
    save();
    render();
    toast(`Quest added for ${name}`, `已给 ${name} 添加任务`);
  };

  window.editManagedTask = function editManagedTask(id) {
    const name = adminTarget();
    const task = DATA[name].tasks.find((item) => item[5] === id);
    if (!task) return;
    const title = prompt('任务名称', task[0]);
    if (title === null || !title.trim()) return;
    const chinese = prompt('中文说明', task[1] || '');
    if (chinese === null) return;
    const xp = prompt('XP 奖励', task[2]);
    if (xp === null) return;
    const coins = prompt('金币奖励', task[3]);
    if (coins === null) return;
    task[0] = title.trim();
    task[1] = chinese.trim();
    task[2] = Math.max(0, Number(xp) || 0);
    task[3] = Math.max(0, Number(coins) || 0);
    save();
    render();
    toast('Quest updated', '任务已修改');
  };

  window.removeManagedTask = function removeManagedTask(id) {
    const name = adminTarget();
    const index = DATA[name].tasks.findIndex((task) => task[5] === id);
    if (index < 0 || !confirm(`删除 ${DATA[name].tasks[index][0]}？`)) return;
    delete state.done[`${name}-${id}`];
    DATA[name].tasks.splice(index, 1);
    save();
    render();
    toast('Quest removed', '任务已删除');
  };
